...
def set_directory():
    ...
    main_path = os.getcwd()
    absolute_path = main_path + "/Stacking_CAROC_Results"
    try:
...
def plot_analysis(model, X_test, y_test, columns, name):
    ...
    plt.savefig(f'Stacking_CAROC_Results/Stacking_Permutation_Importance_{name}.png')
    plt.clf()
    plt.close
...
def plot_predictions(model, X_train, X_test, y_train, y_test, name):
    ...
    viz.show(outpath=f'Stacking_CAROC_Results/Stacking_Class_Prediction_Error_{name}.png')
    plt.clf()
    ...
    viz.fit(X_train, y_train)
    viz.score(X_test, y_test)
    viz.show(outpath=f'Stacking_CAROC_Results/Stacking_confusion_matrix_{name}.png')
    plt.clf()
    ...
    viz.fit(X_train, y_train)
    viz.score(X_test, y_test)
    viz.show(outpath=f'Stacking_CAROC_Results/Stacking_classification_report_{name}.png')
    plt.clf()
    ...
    viz.fit(X_train, y_train)
    viz.score(X_test, y_test)
    viz.show(outpath=f'Stacking_CAROC_Results/Stacking_ROCAUC_{name}.png')
    plt.clf()
...
def plot_high_probability_to_tscore(y_prob, tscore, gender, name):
    ...
    plt.grid(True)
    plt.savefig(f'Stacking_CAROC_Results/Stacking_High_Risk_Probability_vs_Tscore_{name}.png')
    plt.clf()
    plt.close
...
def build_run_and_plot(path, name):
    ...
    #oversample = SMOTENC(categorical_features=[1, 4, 5, 6], random_state=1)
    #X_train, y_train = oversample.fit_resample(X_train, y_train)

    svc = SVC()
    svc_list = {'C': np.arange(1, 50, 1),
    ...
    svc_search = RandomizedSearchCV(estimator=svc,
                                    ...
                                    cv=10,
                                    random_state=20,)
    svc_search.fit(X_train[svc_columns], y_train['bmdtest_10yr_caroc'])
    ...
    rnd_search = RandomizedSearchCV(estimator=rnd,
                                    ...
                                    verbose=1,
                                    random_state=5,)
    rnd_search.fit(X_train[rfc_columns],  y_train['bmdtest_10yr_caroc'])
    ...
    mlp = MLPClassifier(alpha=0.5,
                        ...
                        random_state=1)
    mlp.fit(X_train[mlp_columns], y_train['bmdtest_10yr_caroc'])
...
if __name__ == '__main__':
    ...
    try:
        build_run_and_plot(file_name, 'CAROC')
    except ValueError as er:
        logging.error(er)
